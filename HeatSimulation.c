...
double heat_equ(double px, double py, double t, double **T, double Lx, double Ly)
{
	...
	for (int m = 1; m < N; m++) {
		...
		for (int psi = 0; psi < Ly; psi++) {
		    tmp2 += T[phi][psi] * sin((double)n * (M_PI/Lx) * (double)phi) * sin((double)m * (M_PI/Ly) * (double)psi);
		}
	    }
...
int main()
{
	...
	for (int x = 0; x < wind.grid; x++) {
		...
		double temp = heat_equ(x, y, t, T, wind.grid, wind.grid);
		...
		DrawRectangle(wind.Offset.x + (x*wind.x), wind.Offset.y + (y*wind.y), wind.x, wind.y, (Color){tmp*0xff, 0x00, tmp*0xff, 0xff});
	    }
	    printf("\n");
	}
	printf("-------------------------------------------------------------------------------\n");
